...
class inputbox:
    def __init__(self, x, y, pic, num):
        self.x = x
        self.y = y
        self.rect = Rect(x, y, 58, 20)
        self.image = pic
        self.active = False
        self.num = num
...
while True:
    ...
    if resize:
        ...
        #
        # blink = myfont.render('|', False, (0, 0, 0))

        screen = pygame.display.set_mode((16 * width + 24, 16 * height + 94), 0, 32)
        pygame.display.set_caption('mine sweeper')

        pic = pygame.image.load('cloneskin.png').convert()

        customPic = pygame.image.load('custom.png').convert()
        ...
        v = []
        for i in range(15):
            v.append(pic.subsurface(i % 9 * 16, i // 9 * 16, 16, 16))
        Gpic = dict(zip(k, v))
        ...
        v = []
        for i in range(5):
            v.append(pic.subsurface(27 * i, 55, 25, 25))
        Fpic = dict(zip(k, v))
        facePos = ((16 * width + 24) / 2 - 12.5, 41)
        faceRect = Rect(facePos, (25, 25))

        panel = pic.subsurface(28, 82, 41, 25)
        panelPos1 = (17, 41)
        panelPos2 = (16 * width - 34, 41)

        t = []
        for i in range(11):
            t.append(pic.subsurface(i * 12, 33, 11, 21))
        num_pos1 = (19, 43)
        num_pos2 = (32, 43)
        num_pos3 = (45, 43)
        ...
        lefttop = pic.subsurface(0, 82, 12, 11)
        leftmiddle = pic.subsurface(0, 96, 12, 11)
        leftmiddle1 = pic.subsurface(0, 94, 12, 1)
        leftbottom = pic.subsurface(0, 110, 12, 11)
        righttop = pic.subsurface(15, 82, 12, 11)
        rightmiddle = pic.subsurface(15, 96, 12, 11)
        rightmiddle1 = pic.subsurface(15, 94, 12, 1)
        rightbottom = pic.subsurface(15, 110, 12, 11)
        middletop = pic.subsurface(13, 82, 1, 11)
        onepixel = pic.subsurface(70, 82, 1, 1)

        cuspanel = customPic.subsurface(0, 0, 152, 155)
        ok_up = customPic.subsurface(153, 48, 58, 23)
        ok_press = customPic.subsurface(153, 0, 58, 23)
        cancel_up = customPic.subsurface(153, 72, 58, 23)
        cancel_press = customPic.subsurface(153, 24, 58, 23)
        input_box = customPic.subsurface(153, 96, 58, 20)

        cur = pygame.image.load('m.png').convert_alpha()

        input_b = [inputbox(78, 94, input_box, height),
                   inputbox(78, 129, input_box, width),
                   inputbox(78, 163, input_box, mines)]
            ...
            if event.type == pygame.QUIT:
                ...
                if menu == True:
                    ...
                    if sum(event.buttons) == 0:
                        for i in input_b:
                            if i.rect.collidepoint(event.pos):
                                screen.blit(saveCustom, (0, 67))
                                pygame.mouse.set_visible(False)
                                screen.blit(cur, event.pos)
                                break
                            else:
                                screen.blit(saveCustom, (0, 67))
                    ...
                    for m1 in mlist[1:]:
                        if m1.rect.collidepoint(event.pos):
                            ...
                            if width == -1:
                                ...
                                screen.fill((255, 255, 255), mlist[0].rect)
                                screen.blit(mlist[0].text, (mlist[0].x + 5, mlist[0].y + 5))
                                savePanel = screen.subsurface(0, 67, 151, 154).copy()
                                screen.blit(cuspanel, (0, 67))
                                saveCustom = screen.subsurface(0, 67, 151, 154).copy()
                            resize = True
